...
namespace Conversion
{
    class MyHook
    {
        ...
        //flag=0 正常  flag=1 监控状态  flag=2 截获/截断(屏蔽键盘)//
            ...
            if ((nCode >= 0) && (this.OnKeyDown != null || this.OnKeyUp!= null || this.OnKeyPress!= null))
            {
                ...
                // raise KeyPress
                    if (this.OnKeyPress != null && wParam == (int)WM_KEYBOARD.WM_KEYDOWN)
                    {
                        ...
                        try
                        {
                             isDownShift = ((Win32API.GetKeyState(VK_SHIFT) & 0x80) == 0x80 ? true : false);
                            isDownCapslock = (Win32API.GetKeyState(VK_CAPITAL) != 0 ? true : false);
                        }
                        catch
                        {
